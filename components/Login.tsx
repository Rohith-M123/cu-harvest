  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(`Login failed: ${error.message || 'Invalid credentials'}. Please try: admin@cu-harvest.com / admin123`);
      }
    } catch (error) {
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        setError(`Registration failed: ${error.message || 'Validation error'}. Please ensure: Name (2+ chars), Valid email, Password (6+ chars), Phone (10-15 digits)`);
      }
    } catch (error) {
      console.error('Network error:', error);
      setError('Network error. Please check if backend is running.');
